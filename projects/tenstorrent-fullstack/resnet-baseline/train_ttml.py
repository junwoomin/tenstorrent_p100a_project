...
from resnet_ttml import FrozenResNet, ResNetClassifier
...
def main():
    if RESNET_DEPTH not in (18, 50, 101):
        raise ValueError("RESNET_DEPTH는 18, 50, 101 중 하나여야 합니다.")
    print(f"모델: ResNet-{RESNET_DEPTH} | 저장 위치: {SAVE_DIR}")
    random.seed(SEED)
    np.random.seed(SEED)
